	...
	class MemberOrder
	{
		...
		function getMemberOrderByID($id)
		{
			...
			if($dbobj)
			{
				...
				$this->business_name = $dbobj->business_name;
				$this->parentcorporate = $dbobj->parentcorporate;
				$this->businessowner = $dbobj->business_owner;
				$this->primarycontcatname = $dbobj->primary_contct_name;
				$this->mobiledirectory = $dbobj->mobile_directory;
				$this->companymissionstatement = $dbobj->company_mission_statement;
				$this->coachingcompany = $dbobj->coaching_company;
				$this->categorytypecoach = $dbobj->category_type_coach;
				$this->coachingspecilities = $dbobj->coaching_specilities;
				$this->yearfounded = $dbobj->year_founded;
				$this->professionalassociations = $dbobj->professional_associations;
				$this->whatisyourwhy = $dbobj->what_is_your_why;
				$this->companywebsite = $dbobj->company_name;
				$this->sociallinkedin = $dbobj->social_linkedin;
				$this->socialfacebook = $dbobj->social_facebook;
				$this->socialinstagram = $dbobj->social_instagram;
				$this->socialyoutube = $dbobj->social_youtube;
				$this->socialother = $dbobj->social_other;
				$this->positivereview = $dbobj->positive_review;
				$this->bustitle = $dbobj->title;
		...
		*/
		...
		function saveOrder()
		{
			...
			if(empty($this->parentcorporate))
				$this->parentcorporate = "";
			
			if(empty($this->businessowner))
				$this->businessowner = "";
			
			if(empty($this->primarycontcatname))
				$this->primarycontcatname = "";

			if(empty($this->mobiledirectory))
				$this->mobiledirectory = "";

			if(empty($this->companymissionstatement))
				$this->companymissionstatement = "";

			if(empty($this->coachingcompany))
				$this->coachingcompany = "";

			if(empty($this->categorytypecoach))
				$this->categorytypecoach = "";

			if(empty($this->coachingspecilities))
				$this->coachingspecilities = "";

			if(empty($this->yearfounded))
					$this->yearfounded = "";

			if(empty($this->professionalassociations))
				$this->professionalassociations = "";

			if(empty($this->whatisyourwhy))
				$this->whatisyourwhy = "";

			if(empty($this->companywebsite))
					$this->companywebsite = "";

			if(empty($this->sociallinkedin))
							$this->sociallinkedin = "";

			if(empty($this->socialfacebook))
							$this->socialfacebook = "";

			if(empty($this->socialinstagram))
							$this->socialinstagram = "";

			if(empty($this->socialyoutube))
							$this->socialyoutube = "";

			if(empty($this->socialother))
							$this->socialother = "";

			if(empty($this->positivereview))
							$this->positivereview = "";

			if(empty($this->bustitle))
							$this->bustitle = "";
...
if(empty($this->checkout_id) || intval($this->checkout_id)<1) {
			...
			if(!empty($this->id))
			{
				...
				$this->sqlQuery = "UPDATE $wpdb->pmpro_membership_orders
									...
									`business_name` = '" . esc_sql($this->business_name) . "',
									`parentcorporate` = '" . esc_sql($this->parentcorporate) . "',
									`business_owner` = '" . esc_sql($this->businessowner) . "',
									`primary_contct_name` = '" . esc_sql($this->primarycontcatname) . "',
									`mobile_directory` = '" . esc_sql($this->mobiledirectory) . "',
									`company_mission_statement` = '" . esc_sql($this->companymissionstatement) . "',
									`coaching_company` = '" . esc_sql($this->coachingcompany) . "',
									`category_type_coach` = '" . esc_sql($this->categorytypecoach) . "',
									`coaching_specilities` = '" . esc_sql($this->coachingspecilities) . "',
									`year_founded` = '" . esc_sql($this->yearfounded) . "',
									`professional_associations` = '" . esc_sql($this->professionalassociations) . "',
									`what_is_your_why?` = '" . esc_sql($this->whatisyourwhy) . "',
									`company_name` = '" . esc_sql($this->companywebsite) . "',
									`social_linkedin` = '" . esc_sql($this->sociallinkedin) . "',
									`social_facebook` = '" . esc_sql($this->socialfacebook) . "',
									`social_instagram` = '" . esc_sql($this->socialinstagram) . "',
									`social_youtube` = '" . esc_sql($this->socialyoutube) . "',
									`social_other` = '" . esc_sql($this->socialother) . "',
									`positive_review` = '" . esc_sql($this->positivereview) . "',
									`title` = '" . esc_sql($this->bustitle) . "',
				...
				$this->sqlQuery = "INSERT INTO $wpdb->pmpro_membership_orders
								...
								VALUES('" . esc_sql( $this->code ) . "',
									   ...
									   '" . esc_sql($this->affiliate_subid) . "',
										'" . esc_sql($this->notes) . "',
										'" . esc_sql($_POST['business_name']) . "',
										'" . esc_sql($_POST['parentcorporate']) . "',
										'" . esc_sql($_POST['businessowner']) . "',
										'" . esc_sql($_POST['primarycontcatname']) . "',
										'" . esc_sql($_POST['mobiledirectory']) . "',
										'" . esc_sql($_POST['companymissionstatement']) . "',
										'" . esc_sql($_POST['coachingcompany']) . "',
										'" . esc_sql($_POST['categorytypecoach']) . "',
										'" . esc_sql($_POST['coachingspecilities']) . "',
										'" . esc_sql($_POST['yearfounded']) . "',
										'" . esc_sql($_POST['professionalassociations']) . "',
										'" . esc_sql($_POST['whatisyourwhy']) . "',
										'" . esc_sql($_POST['companywebsite']) . "',
										'" . esc_sql($_POST['sociallinkedin']) . "',
										'" . esc_sql($_POST['socialfacebook']) . "',
										'" . esc_sql($_POST['socialinstagram']) . "',
										'" . esc_sql($_POST['socialyoutube']) . "',
										'" . esc_sql($_POST['socialother']) . "',
										'" . esc_sql($_POST['positivereview']) . "',
										'" . esc_sql($_POST['bustitle']) . "',
										'" . esc_sql($_POST["coachimg"]) . "',
