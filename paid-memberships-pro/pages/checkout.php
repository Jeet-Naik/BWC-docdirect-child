...
$training_path = time().'_'.$_FILES['upload_file']['name'];
...
if($training_path){
	...
	//move_uploaded_file($tmp_file_name,$upload_dir['basedir'].'\file\'.$training_path);
}
...
<form id="pmpro_form" class="<?php echo pmpro_get_element_class( 'pmpro_form' ); ?>" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
	<input type="hidden" id="level" name="level" value="<?php echo esc_attr($pmpro_level->id) ?>" /> <input type="hidden" id="checkjavascript" name="checkjavascript" value="1" /><input type="hidden" id="businessname" name="business_name" value="<?php echo esc_attr($_POST['businessname']) ?>" /> <input type="hidden" id="parentcorporate" name="parentcorporate" value="<?php echo esc_attr($_POST['parent_corporate']) ?>" /> <input type="hidden" name="coachimg" value="<?php echo esc_attr($_FILES["upload_file"]["name"]) ?>"> <input type="hidden" id="businessowner" name="businessowner" value="<?php echo esc_attr($_POST['business_owner']) ?>" /> <input type="hidden" id="primarycontct_name" name="primarycontcatname" value="<?php echo esc_attr($_POST['primary_contct_name']) ?>" /> <input type="hidden" id="mobiledirectory" name="mobiledirectory" value="<?php echo esc_attr($_POST['mobile_directory']) ?>" /> <input type="hidden" id="companymission_statement" name="companymissionstatement" value="<?php echo esc_attr($_POST['company_mission_statement']) ?>" /> <input type="hidden" id="coachingompany" name="coachingcompany" value="<?php echo esc_attr($_POST['coaching_company']) ?>" /> <input type="hidden" id="categorytype_coach" name="categorytypecoach" value="<?php echo esc_attr($_POST['category_type_coach']) ?>" /> <input type="hidden" id="coachingspecilities" name="coachingspecilities" value="<?php echo esc_attr($_POST['coaching_specilities']) ?>" /> <input type="hidden" id="yearfounded" name="yearfounded" value="<?php echo esc_attr($_POST['year_founded']) ?>" /> <input type="hidden" id="professional_associations" name="professionalassociations" value="<?php echo esc_attr($_POST['professional_associations']) ?>" /> <input type="hidden" id="whatisyourwhy" name="whatisyourwhy" value="<?php echo esc_attr($_POST['what_is_your_why']) ?>" /> <input type="hidden" id="company_website" name="companywebsite" value="<?php echo esc_attr($_POST['company_website']) ?>" /> <input type="hidden" id="social_linkedin" name="sociallinkedin" value="<?php echo esc_attr($_POST['social_linkedin']) ?>" /> <input type="hidden" id="social_facebook" name="socialfacebook" value="<?php echo esc_attr($_POST['social_facebook']) ?>" /> <input type="hidden" id="social_instagram" name="socialinstagram" value="<?php echo esc_attr($_POST['social_instagram']) ?>" /> <input type="hidden" id="social_youtube" name="socialyoutube" value="<?php echo esc_attr($_POST['social_youtube']) ?>" /> <input type="hidden" id="social_other" name="socialother" value="<?php echo esc_attr($_POST['social_other']) ?>" /> <input type="hidden" id="positive_review" name="positivereview" value="<?php echo esc_attr($_POST['positive_review']) ?>" /> <input type="hidden" id="title" name="bustitle" value="<?php echo esc_attr($_POST['title']) ?>" />
	<?php if ($discount_code && $pmpro_review) { ?> <input class="<?php echo pmpro_get_element_class( 'input pmpro_alter_price', 'discount_code' ); ?>" id="discount_code" name="discount_code" type="hidden" size="20" value="<?php echo esc_attr($discount_code) ?>" />
	<?php } ?>
	...
	<?php
		...
		<div id="pmpro_pricing_fields" class="<?php echo pmpro_get_element_class( 'pmpro_checkout', 'pmpro_pricing_fields' ); ?>">
			...
			<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-fields' ); ?>">
				...
				</div><?php do_action("pmpro_checkout_after_level_cost"); ?>
				...
				<div id="other_discount_code_tr" style="display: none;">
					<label for="other_discount_code"><?php _e('Discount Code', 'paid-memberships-pro' );?></label> <input id="other_discount_code" name="other_discount_code" type="text" class="<?php echo pmpro_get_element_class( 'input pmpro_alter_price', 'other_discount_code' ); ?>" size="20" value="<?php echo esc_attr($discount_code); ?>" />
	...
	<div id="pmpro_user_fields" class="<?php echo pmpro_get_element_class( 'pmpro_checkout', 'pmpro_user_fields' ); ?>">
		...
		<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-fields' ); ?>">
			...
			</div><?php do_action('pmpro_checkout_after_username'); ?>
						...
						<?php
							...
							<?php } elseif($state_dropdowns == "abbreviations") {
								...
								<select name="bstate" class="<?php echo pmpro_get_element_class( '', 'bstate' ); ?>">
									<option value="">--</option>
									<?php
										foreach($pmpro_states_abbvv5reviations as $ab)
										{
									?>
		...
		<div id="pmpro_payment_information_fields" class="<?php echo pmpro_get_element_class( 'pmpro_checkout', 'pmpro_payment_information_fields' ); ?>" <?php if(!$pmpro_requirebilling || apply_filters("pmpro_hide_payment_information_fields", false) ) { ?>style="display: none;"<?php } ?>>
			...
			<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-fields' ); ?>">
				<?php
					...
					<?php } else { ?>
						<input type="hidden" id="CardType" name="CardType" value="<?php echo esc_attr($CardType);?>" />	<?php } ?>
	...
	<div class="<?php echo pmpro_get_element_class( 'pmpro_submit' ); ?>">
		...
		<?php
				...
				<span id="pmpro_submit_span">
					...
					<input type="submit" name="submitdata" id="pmpro_btn-submit" class="<?php echo pmpro_get_element_class(  'pmpro_btn pmpro_btn-submit-checkout', 'pmpro_btn-submit-checkout' ); ?>" value="<?php if($pmpro_requirebilling) { _e('Submit and Check Out', 'paid-memberships-pro' ); } else { _e('Submit and Confirm', 'paid-memberships-pro' );}?> &raquo;" />
				</span>
				<?php
				}
			?>
