...
if ( ! empty( $_REQUEST['save'] ) ) {
	...
	if ( ! in_array( 'business_name', $read_only_fields ) && isset( $_POST['business_name'] ) ) {
		$order->business_name = sanitize_text_field( $_POST['business_name'] );
	}
	if ( ! in_array( 'parentcorporate', $read_only_fields ) && isset( $_POST['parentcorporate'] ) ) {
		$order->parentcorporate = sanitize_text_field( $_POST['parentcorporate'] );
	}
	if ( ! in_array( 'business_owner', $read_only_fields ) && isset( $_POST['business_owner'] ) ) {
		$order->businessowner = sanitize_text_field( $_POST['business_owner'] );
	}
	if ( ! in_array( 'primary_contct_name', $read_only_fields ) && isset( $_POST['primary_contct_name'] ) ) {
		$order->primarycontcatname = sanitize_text_field( $_POST['primary_contct_name'] );
	}
	if ( ! in_array( 'mobile_directory', $read_only_fields ) && isset( $_POST['mobile_directory'] ) ) {
		$order->mobiledirectory = sanitize_text_field( $_POST['mobile_directory'] );
	}
	if ( ! in_array( 'company_mission_statement', $read_only_fields ) && isset( $_POST['company_mission_statement'] ) ) {
		$order->companymissionstatement = sanitize_text_field( $_POST['company_mission_statement'] );
	}
	if ( ! in_array( 'coaching_company', $read_only_fields ) && isset( $_POST['coaching_company'] ) ) {
		$order->coachingcompany = sanitize_text_field( $_POST['coaching_company'] );
	}
	if ( ! in_array( 'category_type_coach', $read_only_fields ) && isset( $_POST['category_type_coach'] ) ) {
		$order->categorytypecoach = sanitize_text_field( $_POST['category_type_coach'] );
	}
	if ( ! in_array( 'coaching_specilities', $read_only_fields ) && isset( $_POST['coaching_specilities'] ) ) {
		$order->coachingspecilities = sanitize_text_field( $_POST['coaching_specilities'] );
	}
	if ( ! in_array( 'year_founded', $read_only_fields ) && isset( $_POST['year_founded'] ) ) {
		$order->yearfounded = sanitize_text_field( $_POST['year_founded'] );
	}
	if ( ! in_array( 'professional_associations', $read_only_fields ) && isset( $_POST['professional_associations'] ) ) {
		$order->professionalassociations = sanitize_text_field( $_POST['professional_associations'] );
	}
	if ( ! in_array( 'what_is_your_why?', $read_only_fields ) && isset( $_POST['what_is_your_why?'] ) ) {
		$order->whatisyourwhy = sanitize_text_field( $_POST['what_is_your_why?'] );
	}
	if ( ! in_array( 'company_website', $read_only_fields ) && isset( $_POST['company_website'] ) ) {
		$order->companywebsite = sanitize_text_field( $_POST['company_website'] );
	}
	if ( ! in_array( 'social_linkedin', $read_only_fields ) && isset( $_POST['social_linkedin'] ) ) {
		$order->sociallinkedin = sanitize_text_field( $_POST['social_linkedin'] );
	}
	if ( ! in_array( 'social_facebook', $read_only_fields ) && isset( $_POST['social_facebook'] ) ) {
		$order->socialfacebook = sanitize_text_field( $_POST['social_facebook'] );
	}
	if ( ! in_array( 'social_instagram', $read_only_fields ) && isset( $_POST['social_instagram'] ) ) {
		$order->socialinstagram = sanitize_text_field( $_POST['social_instagram'] );
	}
	if ( ! in_array( 'social_youtube', $read_only_fields ) && isset( $_POST['social_youtube'] ) ) {
		$order->socialyoutube = sanitize_text_field( $_POST['social_youtube'] );
	}
	if ( ! in_array( 'social_other', $read_only_fields ) && isset( $_POST['social_other'] ) ) {
		$order->socialother = sanitize_text_field( $_POST['social_other'] );
	}
	if ( ! in_array( 'positive_review', $read_only_fields ) && isset( $_POST['positive_review'] ) ) {
		$order->positivereview = sanitize_text_field( $_POST['positive_review'] );
	}
	if ( ! in_array( 'title', $read_only_fields ) && isset( $_POST['title'] ) ) {
		$order->bustitle = sanitize_text_field( $_POST['title'] );
	}
	...
	if ( ! empty( $_REQUEST['order'] ) ) {
		...
		if ( $order_id > 0 ) {
			...
			$order->notes = '';
			$order->business_name = '';
			$order->parentcorporate ='';
			$order->businessowner ='';
			$order->primarycontcatname ='';
			$order->mobiledirectory = "";
			$order->companymissionstatement = "";
			$order->coachingcompany = "";
			$order->categorytypecoach = "";
			$order->coachingspecilities = "";
			$order->yearfounded = "";
			$order->professionalassociations = "";
			$order->whatisyourwhy = "";
			$order->companywebsite = "";
			$order->sociallinkedin = "";
			$order->socialfacebook = "";
			$order->socialinstagram = "";
			$order->socialyoutube = "";
			$order->socialother = "";
			$order->positivereview = "";
			$order->bustitle = "";
		}
	}
}
...
<?php if ( ! empty( $order ) ) { ?>
	...
	<form method="post" action="">
		...
		<tr>
				<th scope="row" valign="top"><label for="business"><?php esc_html_e( 'Business Name', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'business_name', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->business_name );
					} else { ?>
								<input id="business" name="business_name" type="text" size="50"
												   value="<?php echo esc_attr( $order->business_name ); ?>"/>
					<?php } ?>
				</td>
			</tr>
			<tr>
				<th scope="row" valign="top"><label for="parentcorporate"><?php esc_html_e( 'Parent Corporate', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'parentcorporate', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->parentcorporate );
					} else { ?>
							<input id="parentcorporate" name="parentcorporate" type="text" size="50"
												   value="<?php echo esc_attr( $order->parentcorporate ); ?>"/>
					<?php } ?>
				</td>
			</tr>
			<tr>
				<th scope="row" valign="top"><label for="business_owner"><?php esc_html_e( 'Business Owner', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'business_owner', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->businessowner );
					} else { ?>
							<input id="business_owner" name="business_owner" type="text" size="50"
												   value="<?php echo esc_attr( $order->businessowner ); ?>"/>
					<?php } ?>
				</td>
			</tr>
			<tr>
				<th scope="row" valign="top"><label for="primary_contct_name"><?php esc_html_e( 'Primary Contcat Name', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'primary_contct_name', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->primarycontcatname );
					} else { ?>
							<input id="primary_contct_name" name="primary_contct_name" type="text" size="50"
												   value="<?php echo esc_attr( $order->primarycontcatname ); ?>"/>
					<?php } ?>
				</td>
			</tr>

			<tr>
				<th scope="row" valign="top"><label for="mobile_directory"><?php esc_html_e( 'Mobile Directory', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'mobile_directory', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->mobiledirectory );
					} else { ?>
							<input id="mobile_directory" name="mobile_directory" type="text" size="50"
												   value="<?php echo esc_attr( $order->mobiledirectory ); ?>"/>
					<?php } ?>
				</td>
			</tr>
			<tr>
				<th scope="row" valign="top"><label for="parentcorporate"><?php esc_html_e( 'Company Mission Statement', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					...
					if ( in_array( 'company_mission_statement', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->companymissionstatement );
					} else { ?>
							<input id="company_mission_statement" name="company_mission_statement" type="text" size="50"
												   value="<?php echo esc_attr( $order->companymissionstatement ); ?>"/>
					<?php } ?>
				</td>
			</tr>
			<tr>
				<th scope="row" valign="top"><label for="coaching_company"><?php esc_html_e( 'Coaching Company', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'coaching_company', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->coachingcompany );
					} else { ?>
							<input id="coaching_company" name="coaching_company" type="text" size="50"
												   value="<?php echo esc_attr( $order->coachingcompany ); ?>"/>
					<?php } ?>
				</td>
			</tr>
			<tr>
				<th scope="row" valign="top"><label for="category_type_coach"><?php esc_html_e( 'Category Type Coach', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'category_type_coach', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->categorytypecoach );
					} else { ?>
							<input id="category_type_coach" name="category_type_coach" type="text" size="50"
												   value="<?php echo esc_attr( $order->categorytypecoach ); ?>"/>
					<?php } ?>
				</td>
			</tr>
			<tr>
				<th scope="row" valign="top"><label for="coaching_specilities"><?php esc_html_e( 'Coaching Specilities', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'coaching_specilities', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->coachingspecilities );
					} else { ?>
							<input id="coaching_specilities" name="coaching_specilities" type="text" size="50"
												   value="<?php echo esc_attr( $order->coachingspecilities ); ?>"/>
					<?php } ?>
				</td>
			</tr>
			<tr>
				<th scope="row" valign="top"><label for="year_founded"><?php esc_html_e( 'Year Founded', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'year_founded', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->yearfounded );
					} else { ?>
							<input id="year_founded" name="year_founded" type="text" size="50"
												   value="<?php echo esc_attr( $order->yearfounded ); ?>"/>
					<?php } ?>
				</td>
			</tr>
			<tr>
				<th scope="row" valign="top"><label for="professional_associations"><?php esc_html_e( 'Professional Associations', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'professional_associations', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->professionalassociations );
					} else { ?>
							<input id="professional_associations" name="professional_associations" type="text" size="50"
												   value="<?php echo esc_attr( $order->professionalassociations ); ?>"/>
					<?php } ?>
				</td>
			</tr>
			<tr>
				<th scope="row" valign="top"><label for="what_is_your_why"><?php esc_html_e( 'What is your why', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'what_is_your_why', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->whatisyourwhy );
					} else { ?>
							<input id="what_is_your_why" name="what_is_your_why" type="text" size="50"
												   value="<?php echo esc_attr( $order->whatisyourwhy ); ?>"/>
					<?php } ?>
				</td>
			</tr>
			<tr>
				<th scope="row" valign="top"><label for="company_name"><?php esc_html_e( 'Company Website', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'company_name', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->companywebsite );
					} else { ?>
							<input id="company_name" name="company_name" type="text" size="50"
												   value="<?php echo esc_attr( $order->companywebsite ); ?>"/>
					<?php } ?>
				</td>
			</tr>
			<tr>
				<th scope="row" valign="top"><label for="social_linkedin"><?php esc_html_e( 'Linkedin', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'social_linkedin', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->sociallinkedin );
					} else { ?>
							<input id="social_linkedin" name="social_linkedin" type="text" size="50"
												   value="<?php echo esc_attr( $order->sociallinkedin ); ?>"/>
					<?php } ?>
				</td>
			</tr>
			
			<tr>
				<th scope="row" valign="top"><label for="social_facebook"><?php esc_html_e( 'Facebook', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'social_facebook', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->socialfacebook );
					} else { ?>
							<input id="social_facebook" name="social_facebook" type="text" size="50"
												   value="<?php echo esc_attr( $order->socialfacebook ); ?>"/>
					<?php } ?>
				</td>
			</tr>
			<tr>
				<th scope="row" valign="top"><label for="social_instagram"><?php esc_html_e( 'Instagram', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'social_instagram', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->socialinstagram );
					} else { ?>
							<input id="social_instagram" name="social_instagram" type="text" size="50"
												   value="<?php echo esc_attr( $order->socialinstagram ); ?>"/>
					<?php } ?>
				</td>
			</tr>
			<tr>
				<th scope="row" valign="top"><label for="social_youtube"><?php esc_html_e( 'Youtube', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'social_youtube', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->socialyoutube );
					} else { ?>
							<input id="social_youtube" name="social_youtube" type="text" size="50"
												   value="<?php echo esc_attr( $order->socialyoutube ); ?>"/>
					<?php } ?>
				</td>
			</tr>

			<tr>
				<th scope="row" valign="top"><label for="social_other"><?php esc_html_e( 'Other', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'social_other', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->socialother );
					} else { ?>
							<input id="social_other" name="social_other" type="text" size="50"
												   value="<?php echo esc_attr( $order->socialother ); ?>"/>
					<?php } ?>
				</td>
			</tr>

			<tr>
				<th scope="row" valign="top"><label for="positive_review"><?php esc_html_e( 'Positive Review', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'positive_review', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->positivereview );
					} else { ?>
							<input id="positive_review" name="positive_review" type="text" size="50"
												   value="<?php echo esc_attr( $order->positivereview ); ?>"/>
					<?php } ?>
				</td>
			</tr>
			<tr>
				<th scope="row" valign="top"><label for="title"><?php esc_html_e( 'Title', 'paid-memberships-pro' ); ?>
						:</label></th>
				<td>
					<?php
					if ( in_array( 'title', $read_only_fields ) && $order_id > 0 ) {
						echo esc_html( $order->bustitle );
					} else { ?>
							<input id="title" name="title" type="text" size="50"
												   value="<?php echo esc_attr( $order->bustitle ); ?>"/>
					<?php } ?>
				</td>
			</tr>
