...
if ( ! function_exists( 'docdirect_do_process_subscription' ) ) {
	function docdirect_do_process_subscription() {
		...
		if( isset( $_POST['gateway'] ) && $_POST['gateway'] === 'paypal' ){			
			...
			$user_featured_date	= docdirect_get_user_featured_date($user_identity);
			$featured_date	= date('Y-m-d H:i:s');
			
			if( !empty( $user_featured_date['featured_till'] ) && $user_featured_date['featured_till'] > strtotime($featured_date) ){
				$duration = fw_get_db_post_option($package_id, 'duration', true);
				if( $duration > 0 ){
					$featured_date	= strtotime("+".$duration." days", $user_featured_date['featured_till']);
 ...
 * @return array
 */
if (!function_exists('docdirect_get_user_featured_date')) {
	function docdirect_get_user_featured_date($user_id) {
		$user_featured_date    = get_user_meta( $user_id, 'user_featured', true);
		$featured_data	= array();
		$current_date	= date('Y-m-d H:i:s');
		
		$featured_data['is_featured'] = 'no';
		$featured_data['featured_till'] = 0;;
		
		if( !empty( $user_featured_date ) && $user_featured_date == 1 ){
			$featured_expiry    = get_user_meta( $user_id, 'user_featured_expiry', true);
			$featured_data['is_featured'] = 'yes';
		
			if( !empty( $featured_expiry ) ){
				$featured_data['featured_till'] = $featured_expiry;
			}
			
		} else if( !empty( $user_featured_date ) && $user_featured_date > 1 && ( $user_featured_date > strtotime( $current_date ) ) ){
			$featured_data['is_featured'] = 'yes';
			$featured_data['featured_till'] = $user_featured_date;
		} else if( !empty( $user_featured_date ) && $user_featured_date > 1 && ( $user_featured_date < strtotime( $current_date ) ) ){
			$featured_data['is_featured'] = 'no';
			$featured_data['featured_till'] = $user_featured_date;
		} else if( isset( $user_featured_date ) && $user_featured_date === 0 ){
			$featured_expiry    = get_user_meta( $user_id, 'user_featured_expiry', true);
			$featured_data['is_featured'] = 'no';
			$featured_data['featured_till'] = $user_featured_date;
